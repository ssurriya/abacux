...
import 'package:abacux/screens/proforma_invoice/additional_charge_proforma_invoice.dart';
import 'package:abacux/screens/sales_invoice/sales_invoice_additional_charge.dart';
import 'package:abacux/screens/sales_invoice/sales_invoices_screen.dart';
...
import 'add_product_sales_invoices.dart';

class AddOrEditSalesInvoicesScreen extends StatefulWidget {
  static const route = "/add_sales_invoices_screen";
  AddOrEditSalesInvoicesScreen({this.salesInvoiceDetails});

  final SalesInvoiceDetails salesInvoiceDetails;

  @override
  _AddOrEditSalesInvoicesScreenState createState() =>
      _AddOrEditSalesInvoicesScreenState();
}

class _AddOrEditSalesInvoicesScreenState
    extends State<AddOrEditSalesInvoicesScreen> {
  ...
  TextEditingController _paymentDueDate = TextEditingController();
  ...
  int salesInvoiceId; // For Edit Purpose only

  List<LocalProduct> productSalesInvoiceList = [];
  List<String> uuids = [];
  ...
  List<String> _packings = ["Packing", "Shipping"];
  String _selectedPacking;

  List<String> _taxTypes = ["Divinded [-2]", "Test"];
  String _selectedTaxType;

  SalesInvoiceDetails _salesInvoiceDetails;

  bool _isAutoRoundOff = false;

  bool _isTds = false, _isTcs = false, _none = true;

  List<TdsTaxElement> tdsTaxElement = [];
  TdsTaxElement _selectedTdsTax;

  List<TcsTaxType> tcsTaxElement = [];
  TcsTaxType _selectedTcsTax;
  ...
  void initState() {
    ...
    _paymentDueDate.text = dateFormat(_paymentDate);

    // _getProformaProduct();
    // _getAdditionalChargesList();
    _getUserAndCompanyId();
    _getSalesInvoiceProductFromLocal();
    _getAdditionalChargeFromLocal();
    _getLocalChangeValuesFromService();

    _salesInvoiceDetails =
        SalesInvoiceService.getInstance().getSalesInvoiceDetails;
    if (_salesInvoiceDetails == null) {
      if (widget.salesInvoiceDetails != null) {
        setState(() {
          _isEdit = true;
          _isVisible = true;
          _isLoading = true;
          _salesInvoiceDetails = widget.salesInvoiceDetails;
          SalesInvoiceService.getInstance()
              .setSalesInvoiceDetails(_salesInvoiceDetails);
        });
      }
    }
  }
  ...
  //_isEdit = true only
  _loadSalesInvoiceDetails() {
    setState(() {
      _selectedCustomer = _customerList.firstWhere((element) =>
          element.id ==
          _salesInvoiceDetails.saleInvoiceDetailsList[0].customerId);
      //  _billPrefix.text=_salesInvoiceDetails.saleInvoiceDetailsList[0].
    });
  }

  _getLocalChangeValuesFromService() {
    Customer customer = SalesInvoiceService.getInstance().getCustomer;
    DateTime dateTime = SalesInvoiceService.getInstance().getDateTime;
    DateTime paymentDate = SalesInvoiceService.getInstance().getPaymentDate;
    String term = SalesInvoiceService.getInstance().getTerms;
    setState(() {
      if (customer != null) {
      ...
      if (paymentDate != null) {
        ...
        _paymentDueDate.text = _dateFormat(paymentDate);
      }
      if (term != null) {
        _selectedTerms = term;
      }
  ...
  _getSalesInvoiceProductFromLocal() async {
    setState(() {
    ...
    await SalesInvoiceService.getInstance()
      ...
      value.map((e) {
        ...
        print(json);
        setState(() {
          productSalesInvoiceList.add(LocalProduct.fromJson(e));
        });
      }).toList();
    });
    _calculateTotal();
  ...
  _calculateTotal() {
    ...
    setState(() {
      ...
      String discountInAmt = SalesInvoiceService.getInstance().getDiscountInAmt;
      String discountInPercentage =
          SalesInvoiceService.getInstance().getDiscountInPercentage;
      print(discountInPercentage);
      print(discountInAmt);
      // _totalDiscountAmt.text = discountInAmt == null
      //     ? widget.salesInvoice != null
      //         ? widget.salesInvoice.salesinvoices.discountInAmount
      //         : intialvalue.toString()
      //     : discountInAmt;
      // _totalDiscountPrecentage.text = discountInPercentage == null
      //     ? widget.salesInvoice != null
      //         ? widget.salesInvoice.salesinvoices.discountInPercentage
      //         : intialvalue.toString()
  ...
  _getAdditionalChargeFromLocal() async {
    ...
    await SalesInvoiceService.getInstance()
        .getSalesInvoiceAdditionalCharge()
        .then((value) {
      value.map((e) {
        String json = jsonEncode(e);
        print(json);
  ...
  _getTdsTax() async {
    ...
    await DropdownService().getTdsTaxType(body).then((value) {
      print(value);
      setState(() {
        tdsTaxElement = value.tdsTax;
      });
    });
  }
  ...
  _getTcsTax() async {
    ...
    await DropdownService().getTcsTaxType(body).then((value) {
      print(value);
      setState(() {
        tcsTaxElement = value.tcsTaxType;
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () {
        ...
        Navigator.pushReplacementNamed(context, SalesInvoicesScreen.route);
          ...
          leading: IconButton(
              ...
              onPressed: () async {
                Navigator.pushReplacementNamed(
                    context, SalesInvoicesScreen.route);
              }),
          title: Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text(
                !_isEdit ? " Add Sales Invoices" : "Edit Sales Invoices",
                      ...
                      child: IconButton(
                          ...
                          onPressed: () async {
                            _showModelDialog("Delete",
                              ...
                              _deleteProformaInvoiceApi(deleteSalesInvoiceId);
                            });
                          }),
                    )
                  : Container(),
  ...
  Widget _getColumnWidget() {
    return Column(
      children: [
        ...
        dropDownForCustomer(),
        dropDownForTerms(),
        dateTextField(_dateController, "Date", () {
          _selectListDateFrom(context);
        }),
        dateTextField(_paymentDueDate, "Due Date", () {
          _selectPaymentDueDate(context);
        }),
        Padding(
          ...
          child: Row(
            children: [
              InkWell(
                onTap: () {
                  Navigator.pushReplacementNamed(
                      context, AddProductSalesInvoices.route);
                },
                child: Row(
                  children: [
                    Icon(
              ...
              productSalesInvoiceList.isNotEmpty
                  ? GestureDetector(
                      onTap: () {
                        setState(() {
                      ...
                      child: _productAndAdditionalChargeListVisibility(
                          ...
                          productSalesInvoiceList.length),
                    )
                  : Container(),
              productSalesInvoiceList.isEmpty && additionalChargeList.isEmpty
                  ? Padding(
        ...
        productSalesInvoiceList.isEmpty
            ? Container()
            : Visibility(
                ...
                child: ListView.builder(
                  ...
                  itemCount: productSalesInvoiceList.length,
                              ...
                              children: [
                                Row(
                                  ...
                                  children: [
                                        ...
                                        Container(
                                          ...
                                          child: Padding(
                                            padding: const EdgeInsets.only(
                                                left: 8.0),
                                            child: Text(
                                              '${productSalesInvoiceList[index].product}',
                                              style: TextStyle(
                                                  fontSize: 18,
                                    ...
                                    Text(
                                      '\u{20B9} ${productSalesInvoiceList[index].amount}',
                                      style: TextStyle(
                                          fontSize: 20,
                                          letterSpacing: 0.5,
                                          fontWeight: FontWeight.w500),
                                ...
                                Row(
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        Text(
                                          'Qty: ${productSalesInvoiceList[index].quantity}  | ',
                                          style: TextStyle(
                                              fontSize: 15, letterSpacing: 0.5),
                                        ),
                                        Text(
                                          'Price :${productSalesInvoiceList[index].price}',
                                          style: TextStyle(
                                              fontSize: 15, letterSpacing: 0.5),
                                        ),
                                      ],
        ...
        Visibility(
            visible: _isTcs || _isTds,
            child: _isTds
                ? dropDown("Tax Type", _selectedTdsTax, tdsTaxElement,
                    (TdsTaxElement tdsTaxElement) {
                    return tdsTaxElement.taxName;
                  }, onchanged: (TdsTaxElement value) {
                    _selectedTdsTax = value;
                  })
                : dropDown("Tax Type", _selectedTcsTax, tcsTaxElement,
                    (TcsTaxType tcsTaxElement) {
                    return tcsTaxElement.taxType;
                  }, onchanged: (TcsTaxType value) {
                    _selectedTcsTax = value;
                  })),
        Row(
          children: [
        ...
        Row(
          children: [
            checkBox(_isAutoRoundOff, (value) {
              setState(() {
    ...
    return Row(
      children: [
        InkWell(
          onTap: () {
            Navigator.pushReplacementNamed(
                context, SalesInvoiceAdditionalCharge.route);
          },
          child: Row(
            children: [
              Icon(
          ...
          child: Container(
            ...
            child: DropdownSearch<Customer>(
              ...
              onChanged: (Customer customer) async {
                _selectedCustomer = customer;
                await SalesInvoiceService.getInstance().setCustomer(customer);
              },
              mode: Mode.MENU,
        ...
        child: Padding(
          ...
          child: Container(
            ...
            child: DropdownSearch<String>(
              ...
              showSelectedItem: false,
              selectedItem: _selectedTerms,
              onChanged: (String term) async {
                setState(() {
                  _selectedTerms = term;
                  if (term == "10 Days") {
                    _paymentDate =
                        DateTime(_date.year, _date.month, _date.day + 10);
                  } else if (term == "15 Days") {
                    _paymentDate =
                  ...
                  _paymentDueDate.text = dateFormat(_paymentDate);
                  SalesInvoiceService.getInstance()
                      .setPaymentDateTime(_paymentDate);
                  SalesInvoiceService.getInstance().setTerms(term);
                });
              },
              mode: Mode.MENU,
              itemAsString: (String term) => term,
              items: termList,
              showSearchBox: true,
              validator: (text) {
                if (text == null) {
                  return 'Select Any Terms';
                }
                return null;
              },
              searchBoxDecoration: InputDecoration(
      ...
      child: Container(
        ...
        child: Padding(
          ...
          child: Container(
            width: MediaQuery.of(context).size.width - 0.4,
            // margin: EdgeInsets.only(top: 1, bottom: 1),
            child: DropdownSearch<TdsTaxElement>(
              dropdownSearchDecoration: InputDecoration(
                errorStyle: TextStyle(fontSize: 15.0),
                labelText: "Tax Type",
                labelStyle: TextStyle(
              ...
              onChanged: (dynamic type) async {
                _isTds ? _selectedTdsTax = type : _selectedTcsTax = type;
                setState(() {
                  // _isAutoRoundOff = false;
                });
              },
              mode: Mode.MENU,
              itemAsString: (dynamic type) => type.taxName,
              items: _isTds ? tdsTaxElement : tcsTaxElement,
              showSearchBox: true,
              validator: (text) {
                if (text == null) {
        ...
        child: Padding(
          ...
          child: Container(
            ...
            child: DropdownSearch<String>(
              ...
              showSelectedItem: false,
              selectedItem: _selectedPacking,
              onChanged: (String type) async {
                _selectedType = type;
              },
              mode: Mode.MENU,
              itemAsString: (String packing) => packing,
              items: _packings,
              showSearchBox: true,
              validator: (text) {
                if (text == null) {
  ...
  dateTextField(
      TextEditingController controller, String labelText, Function onPressed) {
    return Padding(
      padding: const EdgeInsets.all(4.0),
      child: Container(
        color: Colors.white,
  ...
  Future<Null> _selectListDateFrom(BuildContext context) async {
    ...
    setState(() {
      _date = pickedDate;
      _dateController.text = dateFormat(_date);
      SalesInvoiceService.getInstance().setDateTime(_date);
    });
  }

  Future<Null> _selectPaymentDueDate(BuildContext context) async {
    final DateTime pickedDate = await showDatePicker(
        context: context,
    ...
    setState(() {
      ...
      _paymentDueDate.text = dateFormat(_paymentDate);
      SalesInvoiceService.getInstance().setPaymentDateTime(_paymentDate);
    });
  }
  ...
  createBodyForProformaInvoice() {
    ...
    if (_isEdit) {
      // body["proforma_id"] = salesInvoiceEditUsingId.salesinvoices.id.toString();
  ...
  _clearProformaLocalData({bool isShowToast = false, bool isDelete = false}) {
    ...
    // ProformaInvoiceService.getInstance().setCustomer(null);5
