...
import 'add_or_edit_credit_note_screen.dart';

class AddProductInCreditNote extends StatefulWidget {
  static const route = "/add_or_edit_credit_notes_product";

  const AddProductInCreditNote({this.productValue, this.isUpdate = false});

  final dynamic productValue;
  final bool isUpdate;

  @override
  _AddProductInCreditNoteState createState() => _AddProductInCreditNoteState();
}

class _AddProductInCreditNoteState extends State<AddProductInCreditNote> {
  ...
  _getPurchaseBillProductIndex() async {
    await CreditNoteService.getInstance()
        ...
        .then((value) {
      setState(() {
        creditNoteProductIndex = value.length;
      });
    });
  }
  ...
  _insertPurchaseBillProductToLocal() async {
    ...
    await CreditNoteService.getInstance()
        .insertCreditNoteProductFromLocal(purchaseBillProduct)
        .then((value) {
      Navigator.pushReplacementNamed(context, AddOrEditCreditNoteScreen.route);
    });
  }
  ...
  _updatePurchaseBillProductToLocal() async {
    _loadPurchaseBillProductValue();
    await CreditNoteService.getInstance()
        .updateCreditNoteProductToLocal(
  ...
  _loadPurchaseBillProductValue() {
    purchaseBillProduct = {
      "id": isEdit
          ...
          : widget.isUpdate
              ? creditNoteProductIndex + 1
              : creditNoteProductIndex,
        ...
        leading: IconButton(
            ...
            onPressed: () {
              Navigator.pushReplacementNamed(
                  context, AddOrEditCreditNoteScreen.route);
            }),
        title: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text(
              isEdit ? "Edit Credit Note" : "Add Credit Note",
