...
import 'add_or_edit_warehouse_bin.dart';

class WarehousesBinScreen extends StatefulWidget {
  static const route = "/warehousesbin_screen";
  const WarehousesBinScreen({Key key}) : super(key: key);

  @override
  _WarehousesBinScreenState createState() => _WarehousesBinScreenState();
}

class _WarehousesBinScreenState extends State<WarehousesBinScreen> {
  final GlobalKey flushBarKey = GlobalKey();
  TextEditingController _searchQuery = TextEditingController();

  List<WarehousesBinListElement> _wareHouseList = [];
  ...
  _getWarehouseList() async {
    ...
    await WareHousesBinService.getInstance()
        .wareHouseBinList(body)
        .then((value) {
      print(value);
      _wareHouseList = value.warehousesBinList;
      setState(() {
        _isLoading = true;
      });
    }).catchError((onError) {
  ...
  _sortByName() {
    _wareHouseList.sort((a, b) {
      print("${a.warehouseName}:${b.warehouseName}");
    ...
    _groupByAlphabet(_wareHouseList);
  }

  _groupByAlphabet(List<WarehousesBinListElement> _estimateDetails) {
    groupByAlphabet = groupBy(_estimateDetails, (WarehousesBinListElement obj) {
      return obj.warehouseName[0];
    });
    setState(() {});
  }
        ...
        floatingActionButton: FloatingActionButton(
          onPressed: () async {
            Navigator.pushReplacementNamed(
                context, AddOrEditWarehouseBin.route);
          },
  ...
  _searchOperation(String searchText) {
    print(searchText);
    List<WarehousesBinListElement> filteredCustomer = [];
    filteredCustomer = _wareHouseList
        .where((element) => element.warehouseName
            .toLowerCase()
  ...
  _getWarehouseItem(WarehousesBinListElement warehousesListElement) {
    return GestureDetector(
      onTap: () {
        Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => AddOrEditWarehouseBin(
                  warehousesBinListElement: warehousesListElement,
                )));
      },
      child: Container(
        color: Colors.white,
