...
import 'add_or_edit_quotation_setting.dart';

class QuotationSettingsScreen extends StatefulWidget {
  static const route = "/quotation_settings_screen";
  const QuotationSettingsScreen({Key key}) : super(key: key);

  @override
  _QuotationSettingsScreenState createState() =>
...
class _QuotationSettingsScreenState extends State<QuotationSettingsScreen> {
  ...
  List<QuotationSetting> _estimateDetails = [];
  ...
  _getCustomerList() async {
    ...
    await QuotationSettingService.getInstance()
        .quotationSettingList(body)
        .then((value) {
      _estimateDetails = value.quotationSettings;
      setState(() {
        _isLoading = true;
      });
    }).catchError((onError) {
  ...
  _groupByAlphabet(List<QuotationSetting> _estimateDetails) {
    groupByAlphabet = groupBy(_estimateDetails, (QuotationSetting obj) {
      return obj.prefix[0];
    });
    setState(() {});
  }
        ...
        floatingActionButton: FloatingActionButton(
          onPressed: () async {
            Navigator.pushReplacementNamed(
                context, AddOrEditQuotationSettings.route);
          },
  ...
  _searchOperation(String searchText) {
    print(searchText);
    List<QuotationSetting> filteredCustomer = [];
  ...
  _getWarehouseItem(QuotationSetting estimateSetting) {
    return GestureDetector(
      onTap: () {
        Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => AddOrEditQuotationSettings(
                  quotationSetting: estimateSetting,
                )));
      },
      child: Container(
        color: Colors.white,
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Padding(
                      padding: const EdgeInsets.only(left: 8.0),
                      child: Text(
                        estimateSetting.quotationStartNumber.toString(),
                        style: TextStyle(
                            fontSize: 12,
                            letterSpacing: 0.5,
