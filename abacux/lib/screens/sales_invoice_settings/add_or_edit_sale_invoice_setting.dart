...
import 'sale_invoice_settings_screen.dart';

class AddOrEditSaleInvoiceSettings extends StatefulWidget {
  static const route = "/add_or_edit_sale_invoice_settings";
  AddOrEditSaleInvoiceSettings({this.saleInvoiceSetting});
  final SalesinvoiceSetting saleInvoiceSetting;

  @override
  State<AddOrEditSaleInvoiceSettings> createState() =>
      _AddOrEditSaleInvoiceSettingsState();
}

class _AddOrEditSaleInvoiceSettingsState
    extends State<AddOrEditSaleInvoiceSettings> {
  TextEditingController prefix = TextEditingController();
  TextEditingController saleInvoiceStartNumber = TextEditingController();
  ...
  void initState() {
    ...
    if (widget.saleInvoiceSetting != null) {
      setState(() {
        _isEdit = true;
        prefix.text = widget.saleInvoiceSetting.prefix;
        saleInvoiceStartNumber.text =
            widget.saleInvoiceSetting.salesInvoiceStartNumber.toString();
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
            _isEdit
                ? "Edit Sale Invoice Settings"
                : "Add Sale Invoice Settings",
        ...
        leading: IconButton(
            ...
            onPressed: () {
              Navigator.pushReplacementNamed(
                  context, SaleInvoiceSettingsScreen.route);
            }),
        actions: [
          _isEdit
              ? IconButton(
                  ...
                  onPressed: () async {
                    Map body = {
                      ...
                      "company_id": companyId.toString(),
                      "token": token,
                      "id": widget.saleInvoiceSetting.id.toString(),
                    };
          ...
          IconButton(
              ...
              onPressed: () async {
                Map body = {
                  ...
                  "token": token,
                  "prefix": prefix.text,
                  "sales_invoice_start_number": saleInvoiceStartNumber.text,
                };
                if (_isEdit)
                  body['id'] = widget.saleInvoiceSetting.id.toString();

                print(body);

                _isEdit
                    ? await SaleInvoiceSettingService.getInstance()
                        .editSaleInvoiceSetting(body)
                        .then((value) {
                        _navigate();
                      }).catchError((onError) {})
                    : await SaleInvoiceSettingService.getInstance()
                        .addSaleInvoiceSetting(body)
                        .then((value) {
                        _navigate();
                      }).catchError((onError) {});
              })
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            ...
            textField(saleInvoiceStartNumber, "Sale Invoice Start Number",
                onchanged: (value) {}),
            Row(
  ...
  _navigate() {
    Navigator.pushReplacementNamed(context, SaleInvoiceSettingsScreen.route);
  }
