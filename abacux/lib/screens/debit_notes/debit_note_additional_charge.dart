...
class DebitNoteAdditionalCharges extends StatefulWidget {
  static const route = "/debit_note_additional_charge";

  const DebitNoteAdditionalCharges({this.productValue, this.isUpdate});

  final dynamic productValue;
  final bool isUpdate;

  @override
  _DebitNoteAdditionalChargesState createState() =>
      _DebitNoteAdditionalChargesState();
}

class _DebitNoteAdditionalChargesState
    extends State<DebitNoteAdditionalCharges> {
  ...
  int debitNoteProductIndex = 0;
  int num = 100;
  Map<String, dynamic> debitNoteProduct = {};
  ...
  _getDebitNoteProductIndex() async {
    await DebitNoteService.getInstance()
        .getDebitNoteAdditionalFromLocal()
        .then((value) {
      setState(() {
        print("Indeex ---${value.length}");
        debitNoteProductIndex = value.length;
      });
    });
  }

  _insertDebitNoteProductToLocal() async {
    _loadPurchaseBillProductValue();
    await DebitNoteService.getInstance()
        .insertDebitNoteAdditionalCharges(debitNoteProduct)
        .then((value) {
      Navigator.pushReplacementNamed(context, AddOrEditDebitNoteScreen.route);
    });
  }

  _updateDebitNoteProductToLocal() async {
    _loadPurchaseBillProductValue();
    await DebitNoteService.getInstance()
        .updateDebitNoteAdditionalCharges(debitNoteProduct)
        .then((value) {
      Navigator.pushReplacementNamed(context, AddOrEditDebitNoteScreen.route);
    });
  }
    ...
    debitNoteProduct = {
      "id": isEdit
          ...
          : widget.isUpdate != null
              ? widget.isUpdate
                  ? debitNoteProductIndex + 1
                  : debitNoteProductIndex
              : debitNoteProductIndex,
          ...
          title: Row(
            ...
            children: [
              ...
              GestureDetector(
                onTap: () {
                  isEdit
                      ? _updateDebitNoteProductToLocal()
                      : _insertDebitNoteProductToLocal();
                },
                child: Text(
                  "Save",
