...
import 'add_or_edit_estimate_settings.dart';

class EstimatesSettingsScreen extends StatefulWidget {
  static const route = "/estimate_settings_screen";
  const EstimatesSettingsScreen({Key key}) : super(key: key);

  @override
  _EstimatesSettingsScreenState createState() =>
      _EstimatesSettingsScreenState();
}

class _EstimatesSettingsScreenState extends State<EstimatesSettingsScreen> {
  final GlobalKey flushBarKey = GlobalKey();
  TextEditingController _searchQuery = TextEditingController();

  List<EstimateSetting> _estimateDetails = [];
  ...
  _getCustomerList() async {
    ...
    await EstimateSettingService.getInstance()
        .estimateSettingsList(body)
        .then((value) {
      _estimateDetails = value.estimateSettings;
      setState(() {
        _isLoading = true;
      });
    }).catchError((onError) {
  ...
  _groupByAlphabet(List<EstimateSetting> _estimateDetails) {
    groupByAlphabet = groupBy(_estimateDetails, (EstimateSetting obj) {
      return obj.prefix[0];
    });
    setState(() {});
  }
        ...
        floatingActionButton: FloatingActionButton(
          onPressed: () async {
            Navigator.pushReplacementNamed(
                context, AddOrEditEstimateSettings.route);
          },
  ...
  _searchOperation(String searchText) {
    print(searchText);
    List<EstimateSetting> filteredCustomer = [];
  ...
  _getWarehouseItem(EstimateSetting estimateSetting) {
    return GestureDetector(
      onTap: () {
        Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => AddOrEditEstimateSettings(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Padding(
                      padding: const EdgeInsets.only(left: 8.0),
                      child: Text(
                        estimateSetting.estimateStartNumber.toString(),
                        style: TextStyle(
                            fontSize: 12,
                            letterSpacing: 0.5,
