...
class AddEstimateProductScreen extends StatefulWidget {
  const AddEstimateProductScreen({this.productValue, this.isUpdate = false});

  final dynamic productValue;
  final bool isUpdate;

  @override
  _AddEstimateProductScreenState createState() =>
      _AddEstimateProductScreenState();
}

class _AddEstimateProductScreenState extends State<AddEstimateProductScreen> {
  ...
  _insertEstimateProductToLocal() async {
    _loadEstimateProductValue();
    print(purchaseProduct);
    await EstimateService.getInstance()
        .insertEstimateProductFromLocal(purchaseProduct)
        .then((value) {
  ...
  _updateEstimateProductToLocal() async {
    _loadEstimateProductValue();
    await EstimateService.getInstance()
        .updateEstimateProductToLocal(
            purchaseProduct, widget.productValue['uuid'])
        .then((value) {
      Navigator.pushReplacementNamed(context, '/add_estimate_screen');
    });
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(
          ...
          children: [
            Text(
              isEdit ? "Edit Estimate Items" : "Add Esimate Items",
