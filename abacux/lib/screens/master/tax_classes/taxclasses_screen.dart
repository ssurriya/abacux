...
class TaxClassesScreen extends StatefulWidget {
  static const route = "/taxclass_screen";
  const TaxClassesScreen({Key key}) : super(key: key);

  @override
  _TaxClassesScreenState createState() => _TaxClassesScreenState();
}

class _TaxClassesScreenState extends State<TaxClassesScreen> {
  final GlobalKey flushBarKey = GlobalKey();
  TextEditingController _searchQuery = TextEditingController();

  List<TaxClassesName> _taxClassesList = [];
  ...
  _getUserAndCompanyId() async {
    ...
    _getTaxClassesNameList();
  }

  // Get Customer Listing
  _getTaxClassesNameList() async {
    Map body = {
    ...
    await TaxClassesService.getInstance().TaxClassesList(body).then((value) {
      _taxClassesList = value.taxClassesName;
      setState(() {
        _isLoading = true;
      });
    }).catchError((onError) {
  ...
  _sortByName() {
    _taxClassesList.sort((a, b) {
      print("${a.accountName}:${b.accountName}");
    ...
    _groupByAlphabet(_taxClassesList);
  }

  _groupByAlphabet(List<TaxClassesName> _estimateDetails) {
    groupByAlphabet = groupBy(_estimateDetails, (TaxClassesName obj) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        floatingActionButton: FloatingActionButton(
          onPressed: () async {
            Navigator.pushReplacementNamed(context, AddOrEditTaxClasses.route);
          },
  ...
  _searchOperation(String searchText) {
    print(searchText);
    List<TaxClassesName> filteredCustomer = [];
    filteredCustomer = _taxClassesList
        .where((element) =>
            element.accountName
                .toLowerCase()
  ...
  _getTaxclassesItem(TaxClassesName taxClassesName) {
    return GestureDetector(
      onTap: () {
        Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => AddOrEditTaxClasses(
                  taxClasses: taxClassesName,
                )));
      },
      child: Container(
        color: Colors.white,
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Padding(
                      padding: const EdgeInsets.only(left: 8.0),
                      child: Text(
                        taxClassesName.accountName.toString(),
                        style: TextStyle(
                            fontSize: 12,
                            letterSpacing: 0.5,
