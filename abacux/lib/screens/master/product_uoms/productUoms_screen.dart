...
import 'add_or_edit_productUoms.dart';

class ProductUomsScreen extends StatefulWidget {
  static const route = "/product_uoms_screen";
  const ProductUomsScreen({Key key}) : super(key: key);

  @override
  _ProductUomsScreenState createState() => _ProductUomsScreenState();
}

class _ProductUomsScreenState extends State<ProductUomsScreen> {
  final GlobalKey flushBarKey = GlobalKey();
  TextEditingController _searchQuery = TextEditingController();

  List<Productuom> _productUomsList = [];
  ...
  _getProductuomList() async {
    ...
    await ProductUomsService.getInstance().productUomsList(body).then((value) {
      print(value);
      _productUomsList = value.productuoms;
      setState(() {
        _isLoading = true;
      });
    }).catchError((onError) {
  ...
  _sortByName() {
    _productUomsList.sort((a, b) {
      print("${a.uomName}:${b.uomName}");
      return a.uomName == null
          ? 0
          : b.uomName == null
              ? 0
              : a.uomName.toLowerCase().compareTo(b.uomName.toLowerCase());
    });
    _groupByAlphabet(_productUomsList);
  }

  _groupByAlphabet(List<Productuom> _estimateDetails) {
    groupByAlphabet = groupBy(_estimateDetails, (Productuom obj) {
      return obj.uomName[0];
    });
    setState(() {});
  }
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        floatingActionButton: FloatingActionButton(
          onPressed: () async {
            Navigator.pushReplacementNamed(context, AddOrEditProductUoms.route);
          },
  ...
  _searchOperation(String searchText) {
    print(searchText);
    List<Productuom> filteredCustomer = [];
    filteredCustomer = _productUomsList
        .where((element) =>
            element.uomName.toLowerCase().contains(searchText.toLowerCase()))
        .toList();
    _groupByAlphabet(filteredCustomer);
  }

  _getProductUomsItem(Productuom productuom) {
    return GestureDetector(
      onTap: () {
        Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => AddOrEditProductUoms(
