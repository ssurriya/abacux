...
import 'add_or_edit_tax.dart';

class TaxScreen extends StatefulWidget {
  static const route = "/tax_screen";
...
class _TaxScreenState extends State<TaxScreen> {
  ...
  _sortByName() {
    _taxList.sort((a, b) {
      ...
      return a.tax == null
          ...
          : b.tax == null
              ? 0
              : a.tax.toLowerCase().compareTo(b.tax.toLowerCase());
    });
    _groupByAlphabet(_taxList);
  }

  _groupByAlphabet(List<TaxtypesList> _taxDetails) {
    groupByAlphabet = groupBy(_taxDetails, (TaxtypesList obj) {
      return obj.tax[0];
    });
    setState(() {});
  }
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        floatingActionButton: FloatingActionButton(
          onPressed: () async {
            Navigator.pushReplacementNamed(context, AddOrEditTax.route);
          },
  ...
  _searchOperation(String searchText) {
    print(searchText);
    List<TaxtypesList> filteredTax = [];
    filteredTax = _taxList
        .where((element) =>
            element.tax.toLowerCase().contains(searchText.toLowerCase()))
        .toList();
    _groupByAlphabet(filteredTax);
  }

  _getProductUomsItem(TaxtypesList taxtypesList) {
    return GestureDetector(
      onTap: () {
        Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => AddOrEditTax(
                  taxname: taxtypesList,
                )));
      },
      child: Container(
        color: Colors.white,
